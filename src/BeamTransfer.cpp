...
*/
BeamTransfer::BeamTransfer( MPI_Comm comm,
...
*/
void BeamTransfer::computeRotation( const F2FScalar *q,
                                    const F2FScalar *d,
                                    F2FScalar r[] ){
  // Compute R = (C - I)
  F2FScalar R[9];
...
void BeamTransfer::transferDisps( const F2FScalar *struct_disps,
  ...
  // Set the displacements into Us
  memcpy(Us, struct_disps, ns*dof_per_node*sizeof(F2FScalar));
...
void BeamTransfer::transferLoads( const F2FScalar *aero_loads,
  ...
  // Copy the aerodynamic loads
  memcpy(Fa, aero_loads, 3*na*sizeof(F2FScalar));
  memset(struct_loads, 0, ns*dof_per_node*sizeof(F2FScalar));
  ...
  for ( int i = 0; i < na; i++ ){
    ...
    for ( int k = 0; k < order; k++, nodes++ ){
      const F2FScalar *u = &Us[dof_per_node*nodes[0]];
      F2FScalar *fs = &struct_loads[dof_per_node*nodes[0]];
      fs[0] += N[k]*fa[0];
      fs[1] += N[k]*fa[1];
      fs[2] += N[k]*fa[2];
...
void BeamTransfer::applydDduS( const F2FScalar *vecs,
                               F2FScalar *prods ){
  for ( int i = 0; i < na; i++ ){
    // Get the element and parametric location within the element
    // where the aerodynamic node is attached.
    ...
    for ( int k = 0; k < order; k++, nodes++ ){
      const F2FScalar *u = &Us[dof_per_node*nodes[0]];
      const F2FScalar *v = &vecs[dof_per_node*nodes[0]];
      ua[0] -= N[k]*v[0];
      ua[1] -= N[k]*v[1];
      ua[2] -= N[k]*v[2];
...
void BeamTransfer::applydLduS( const F2FScalar *vecs,
  ...
  for ( int i = 0; i < na; i++ ){
    ...
    for ( int k = 0; k < order; k++, nodes++ ){
      const F2FScalar *v = &vecs[dof_per_node*nodes[0]];
      F2FScalar *fs = &prods[dof_per_node*nodes[0]];
...
void BeamTransfer::applydLduSTrans( const F2FScalar *vecs,
                                    F2FScalar *prods ){
  // Zero the output
  memset(prods, 0, ns*dof_per_node*sizeof(F2FScalar));
...
void BeamTransfer::applydDdxS0( const F2FScalar *vecs, F2FScalar *prods ){
  ...
  for ( int i = 0; i < na; i++ ){
    ...
    for ( int k = 0; k < order; k++, nodes++ ){
      ...
      F2FScalar dr[3];
      computeRotationTranspose(&u[3], ua, dr);
      prods[3*nodes[0]] += N[k]*dr[0];
      prods[3*nodes[0]+1] += N[k]*dr[1];
      prods[3*nodes[0]+2] += N[k]*dr[2];
    }
  }
}


void BeamTransfer::applydLdxA0( const F2FScalar *vecs,
                                F2FScalar *prods ){
  memset(prods, 0, 3*na*sizeof(F2FScalar));
  for ( int i = 0; i < na; i++ ){
    // Get the element and parametric location within the element
    ...
    for ( int k = 0; k < order; k++, nodes++ ){
      const F2FScalar *u = &Us[dof_per_node*nodes[0]];
      const F2FScalar *fs = &vecs[dof_per_node*nodes[0]];
...
void BeamTransfer::applydLdxS0( const F2FScalar *vecs,
                                F2FScalar *prods ){
  memset(prods, 0, 3*ns*sizeof(F2FScalar));
  for ( int i = 0; i < na; i++ ){
    // Get the element and parametric location within the element
    ...
    for ( int k = 0; k < order; k++, nodes++ ){
      ...
      // Compute the contribution from the rigid transformation
      addTransposeRotationDerivAdjoint(N[k], &u[3], fa, &fs[3], &prods[3*nodes[0]]);
    }
  }
}
