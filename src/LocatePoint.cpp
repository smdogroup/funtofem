...
int LocatePoint::locateExhaustive( const F2FScalar xpt[] ){
  ...
  for ( int n = 0; n < npts; n++ ){
    F2FReal t = F2FRealPart((Xpts[3*n]   - xpt[0])*(Xpts[3*n]   - xpt[0]) +
...
*/
void LocatePoint::locateClosestTaxi( int K, int indx[],
				     F2FScalar dist[], const F2FScalar xpt[],
				     const F2FScalar n[] ){
  dist[0] = 1e40;
  indx[0] = -1;
...
void LocatePoint::locateClosest( int root, const F2FScalar xpt[], 
  ...
  if (start != -1){ // This node is a leaf
	...
	*index = n;
    ...
    // The normal distance
    F2FScalar ndist = ((xpt[0] - xav[0])*normal[0] +
...
/*!
  ...
  given point
*/
void LocatePoint::insertIndex( F2FScalar *dist, int *indx, int *nk, 
			       F2FScalar d, int dindex, int K ){
...
void LocatePoint::locateKClosest( int K, int root, const F2FScalar xpt[], 
  ...
  if (start != -1){ // This node is a leaf
    ...
    for ( int k = start; k < end; k++ ){
      ...
      if ((*nk < K ) || 
          (F2FRealPart(t) < F2FRealPart(dist[K-1]))){
	insertIndex(dist, indx, nk, t, n, K );
      }
    }    
  }
...
*/
int LocatePoint::splitList( F2FScalar xav[], F2FScalar normal[], 
                            int * ind, int np ){
  ...
  for ( int i = 0; i < np; i++ ){
    ...
    // I[0] = Ix = y^2 + z^2
    I[0] += F2FRealPart(Xpts[3*n+1]*Xpts[3*n+1] + Xpts[3*n+2]*Xpts[3*n+2]); 
    // I[4] = Iy = x^2 + z^2
    I[4] += F2FRealPart(Xpts[3*n]*Xpts[3*n] + Xpts[3*n+2]*Xpts[3*n+2]);
     // I[8] = Iz = x^2 + y^2
    I[8] += F2FRealPart(Xpts[3*n]*Xpts[3*n] + Xpts[3*n+1]*Xpts[3*n+1]);
