...
RBF::RBF(MPI_Comm all, MPI_Comm structure, int _struct_root,
...
RBF::~RBF() {
  ...
  if (sample_ids) { delete [] sample_ids; }
...
void RBF::initialize() {
  ...
  if (denominator > 1) {
    ...
    Octree *octree = new Octree(Xs, ns, min_point_count, 
                                min_edge_length, max_tree_depth);
    octree->generate();
    nsub = octree->nleaf;
      ...
      for (int j = 0; j < ns; j++) {
        if (octree->points_bins[j] == bin_id) {
          if (num_bin_pts > 0) {
            ...
            memcpy(new_bin_points_ids, bin_points_ids, num_bin_pts*sizeof(int));
...
void RBF::buildInterpolationMatrix() {
  ...
  */
  ...
  BLASgemm("N", "N", &npoly, &nsub, &nsub, &alpha, P, &npoly, 
           invM, &nsub, &beta, Psized, &npoly);
#ifdef FUNTOFEM_USE_COMPLEX
    const char *t = "C";
#else
    const char *t = "T";
#endif
  BLASgemm("N", t, &npoly, &npoly, &nsub, &alpha, Psized, &npoly, P, &npoly, &beta, invMp, &npoly);
  ...
  BLASgemm("N", "N", &npoly, &nsub, &npoly, &alpha, Mp, &npoly, 
           Psized, &npoly, &beta, top_half, &npoly);

  // Use the top half to build the bottom half
  BLASgemm(t, "N", &nsub, &nsub, &npoly, &alpha, P, &npoly, 
           top_half, &npoly, &beta, M, &nsub);
  F2FScalar *bot_half = new F2FScalar[nsub*nsub];
  memcpy(bot_half, invM, nsub*nsub*sizeof(F2FScalar));
  alpha = -1.0; beta = 1.0;
  BLASgemm("N", "N", &nsub, &nsub, &nsub, &alpha, invM, &nsub,
           M, &nsub, &beta, bot_half, &nsub);
  ...
  int k = nsub + npoly;
  alpha = 1.0; beta = 0.0;
  BLASgemm("N", "N", &na, &nsub, &k, &alpha, Aas, &na,
           invCss, &k, &beta, interp_mat, &na);
...
void RBF::transferDisps(const F2FScalar *struct_disps, F2FScalar *aero_disps) {
  ...
  int n = 3;
  F2FScalar alpha = 1.0, beta = 0.0;
  BLASgemm("N", "N", &na, &n, &nsub, &alpha, interp_mat, &na,
           US, &nsub, &beta, UA, &na);
...
void RBF::transferLoads(const F2FScalar *aero_loads, F2FScalar *struct_loads) {
  ...
  F2FScalar alpha = 1.0, beta = 0.0;
    ...
    const char *t = "T";
#endif
  BLASgemm(t, "N", &nsub, &n, &na, &alpha, interp_mat, &na,
           Fxyz, &na, &beta, Fsub, &nsub);
...
F2FScalar RBF::gaussian(F2FScalar *x, F2FScalar *y) {
  F2FScalar sigma = 0.5;
  F2FScalar r = sqrt((x[0] - y[0])*(x[0] - y[0]) + 
                     (x[1] - y[1])*(x[1] - y[1]) + 
                     (x[2] - y[2])*(x[2] - y[2]));
  F2FScalar eval = exp(-0.5*(r/sigma)*(r/sigma))/(sigma*sqrt(2.0*M_PI));
  return eval;
}
...
void RBF::writeCloudsToTecplot() {
    ...
    for (int i = 0; i < ns; i++) {
      fprintf(file, "%22.15e %22.15e %22.15e\n",
              ...
              F2FRealPart(Xs[3*i+1]), 
              F2FRealPart(Xs[3*i+2]));
    }
    ...
    for (int i = 0; i < nsub; i++) {
      ...
      fprintf(file, "%22.15e %22.15e %22.15e\n",
              ...
              F2FRealPart(Xs[3*indx+1]), 
              F2FRealPart(Xs[3*indx+2]));
    }
