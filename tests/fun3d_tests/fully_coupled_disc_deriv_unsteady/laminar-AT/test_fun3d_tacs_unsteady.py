...
class TestFun3dTacsUnsteady(unittest.TestCase):
    ...
    def test_laminar_aerothermal_thick(self):
        ...
        max_rel_error = TestResult.complex_step(
            "fun3d+tacs-laminar-aerothermal-thick",
            model,
            driver,
    ...
    def test_laminar_aerothermal_alpha(self):
        # build the funtofem model with one body and scenario
        model = FUNtoFEMmodel("plate")
        plate = Body.aerothermal("plate", boundary=2)
        plate.register_to(model)
        ...
        # run the complex step test on the model and driver
        max_rel_error = TestResult.complex_step(
            "fun3d+tacs-laminar-aerothermal-alpha",
            model,
            driver,
            TestFun3dTacsUnsteady.FILEPATH,
        )
