...
class TestFun3dTacs(unittest.TestCase):
    ...
    def test_thick_turbulent_aerothermoelastic(self):
        ...
        solvers.flow = Fun3dInterface(comm, model, fun3d_project_name="miniMesh", fun3d_dir="meshes")
    ...
    def test_alpha_turbulent_aerothermoelastic(self):
        # build the funtofem model with one body and scenario
        model = FUNtoFEMmodel("plate")
        plate = Body.aerothermoelastic("plate", boundary=2)
        plate.register_to(model)
