...
class ModelTest(unittest.TestCase):
    def build_model(self):
        ...
        # forward_flight
        forward_flight = Scenario(
            name="forward_flight", group=1, steady=False, fun3d=True, steps=1080
        )

        forward_flight.set_variable(
            "aerodynamic", "AOA", value=5.0, lower=-3.0, upper=10.0, active=True
        )

        thrust = Function(
            "thrust", analysis_type="aerodynamic", start=721, stop=1080, averaging=True
        )
        forward_flight.add_function(thrust)

        thrust = Function(
            "power", analysis_type="aerodynamic", start=721, stop=1080, averaging=True
        )
        forward_flight.add_function(thrust)
        ...
        ks = Function(
            "ksFailure", analysis_type="structural", options=options, averaging=False
        )
        forward_flight.add_function(ks)

        rotor_model.add_scenario(forward_flight)

        # Create the common/shared blade variables
        ...
        for i in range(thicknesses.size):
            ...
            thickness_vars.append(thick)

        # Create the common control variables
        collective = Variable(
            "collective", value=0.2, lower=-0.1, upper=1.0, coupled=True
        )
        cyclic_lat = Variable(
            "cyclic_lat", value=0.1, lower=-0.1, upper=1.0, coupled=True
        )
        cyclic_long = Variable(
            "cyclic_long", value=0.3, lower=-0.1, upper=1.0, coupled=True
        )

        # Set up the bodies
        for bl in range(3):
            blade = Body(
                "blade %i" % bl, group=1, fun3d=True, analysis_type="aeroelastic"
            )

            # Add the coupled thickness variables
            for i in range(thicknesses.size):
                blade.add_variable("structural", thickness_vars[i])

            # Add the uncoupled thickness variable
            thick = Variable(
                "thickness %i" % 4, value=5.0, lower=0.003, upper=5.0, coupled=False
            )
    ...
    def test_set_variables(self):
        ...
        assert (
            model.scenarios[0].variables["aerodynamic"][1].value
            == 3.0 + offset + offset2
        )
        assert (
            model.scenarios[1].variables["aerodynamic"][1].value
            == 5.0 + offset + offset2
        )

        for i in range(3):
            assert (
                model.bodies[i].variables["structural"][0].value
                == 1.0 + offset + offset2
            )
            assert (
                model.bodies[i].variables["structural"][4].value
                == 5.0 + offset + offset2
            )
            assert (
