...
class TacsUnsteadyInterface(SolverInterface):
    ...
    def __init__(self, integrator_options, model=None, ndof=8):
        ...
        if self.tacs_proc:
            ...
            for body in self.tacs_body_list:
                if (
                    body.btype == TACSBodyType.FLEXIBLE
                    or body.btype == TACSBodyType.SOLID
                ):
    ...
    def createTACS(self, options, model, ndof=8):
        ...
        self.tacs = self.builder.getTACS(
            options["ordering"], TACS.PY_DIRECT_SCHUR, ndof=ndof
        )
    ...
    def set_functions(self, scenario, bodies):
        if self.tacs_proc:
            ...
            self.integrator[scenario.id].setFunctions(
                self.funclist, self.ndvs, func.start, func.stop
            )
    ...
    def eval_gradients(self, scenario):
        ...
        if self.tacs_proc:
            ...
            for func in range(len(self.funclist)):
                self.func_grad.append(
                    dfdx[func * self.ndvs : (func + 1) * self.ndvs].copy()
                )
    ...
    def iterate(self, scenario, bodies, step):
        ...
        if self.tacs_proc:
            ...
            for ibody, (tacs_body, body) in enumerate(
                zip(self.tacs_flex_bodies, bodies)
            ):
                for i, n in enumerate(tacs_body.dist_nodes):
                    load_vector[
                        tacs_body.dof * n : tacs_body.dof * n + body.xfer_ndof
                    ] = body.struct_loads[
                        body.xfer_ndof * i : body.xfer_ndof * i + body.xfer_ndof
                    ]
        ...
        if self.tacs_proc:
            ...
            for ibody, (tacs_body, body) in enumerate(
                ...
                for i, n in enumerate(tacs_body.dist_nodes):
                    body.struct_disps[
                        body.xfer_ndof * i : body.xfer_ndof * i + body.xfer_ndof
                    ] = disps[tacs_body.dof * n : tacs_body.dof * n + body.xfer_ndof]
