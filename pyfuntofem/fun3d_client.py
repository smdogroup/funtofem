...
from fun3d.mda.fsi.fun3d_aero import (FUN3DAero, FUN3DAeroException,
...
class Fun3dClient(SolverInterface):
    ...
    def __init__(self,comm,model,flow_dt=1.0,
                 host='localhost', port_base=49200):
        """
        ...
        port = port_base + rank
        context = zmq.Context()
        self.fun3d_client = FUN3DAero.Client(context,
                                             'tcp://%s:%d'%(host,port),
                                             zmq.REQ)
          ...
          if not first_pass:
              for ibody, body in enumerate(bodies,1):
                  if body.shape and body.aero_nnodes > 0:
                    ...
                    self.fun3d_client.set_design_surface(ibody, aero_X, body.aero_id)
          ...
          if first_pass:
            for ibody, body in enumerate(bodies,1):
                ...
                if body.aero_nnodes > 0:
                    mesh = self.fun3d_client.extract_surface(ibody,body.aero_nnodes)
                    body.aero_id = self.fun3d_client.extract_surface_id(ibody,body.aero_nnodes)
        ...
        try:
          for function in scenario.functions:
            if function.adjoint:
                ...
                self.fun3d_client.set_design_composite_function(function.id,
                                                                1,
                                                                start,
                                                                stop,
                                                                1.0,
                ...
                self.fun3d_client.set_design_component_function(function.id,
                                                                1,
                                                                boundary,
                                                                name,
                                                                function.value,
    ...
    def set_variables(self,scenario,bodies):
        ...
        try:
          self.fun3d_client.set_design(len(bodies),scenario.count_adjoint_functions())

          # push the global aerodynamic variables to fun3d
          for var in scenario.variables['aerodynamic']:
            self.fun3d_client.set_design_global_var(var.id,var.active,var.value,var.lower,var.upper)
          ...
          for ibody, body in enumerate(bodies,1):
            ...
            if 'shape' in body.variables:
                for var in body.variables['shape']:
                    self.fun3d_client.set_design_shape_var(ibody,var.id,
                                                           var.active,var.value,
                                                           var.lower,var.upper)

            for var in body.variables['rigid_motion']:
                self.fun3d_client.set_design_rigid_var(ibody,var.id,var.name,
                                                       var.active,var.value,
                                                       var.lower,var.upper)
        except FUN3DAeroException, e:
          print('Error:',e.reason)
          ...
          for function in scenario.functions:
            if function.analysis_type=='aerodynamic':
                ...
                if self.comm.Get_rank() == 0:
                    function.value = self.fun3d_client.get_design_composite_function(function.id)
            ...
            for vartype in scenario.variables:
                if vartype == 'aerodynamic':
                    for i, var in enumerate(scenario.variables[vartype]):
                        if var.active:
                            if function.adjoint:
                                scenario.derivatives[vartype][offset+func][i] = self.fun3d_client.get_design_global_derivative(function.id,var.id)
                            else:
                ...
                for vartype in body.variables:
                    if vartype == 'rigid_motion':
                        for i, var in enumerate(body.variables[vartype]):
                            if var.active:
                                if function.adjoint:
                                    body.derivatives[vartype][offset+func][i] = self.fun3d_client.get_design_rigid_derivative(ibody,function.id,var.id)
                                else:
        ...
        try:
          for ibody, body in enumerate(bodies,1):
            if body.shape and body.aero_nnodes > 0:
                ...
                product = self.fun3d_client.extract_grid_adjoint_product(ibody,body.aero_nnodes,nfunctions)
                product.lam_x = np.array(product.lam_x).reshape((body.aero_nnodes,-1))
                product.lam_y = np.array(product.lam_y).reshape((body.aero_nnodes,-1))
                product.lam_z = np.array(product.lam_z).reshape((body.aero_nnodes,-1))
                body.aero_shape_term[ ::3,:nfunctions] += product.lam_x[:,:] * self.flow_dt
                body.aero_shape_term[1::3,:nfunctions] += product.lam_y[:,:] * self.flow_dt
                body.aero_shape_term[2::3,:nfunctions] += product.lam_z[:,:] * self.flow_dt
        except FUN3DAeroException, e:
          print('Error:',e.reason)
          try:
            self.fun3d_client.popd()
        ...
        try:
          for ibody, body in enumerate(bodies,1):
            if body.aero_nnodes > 0:
                ...
                self.fun3d_client.input_force_adjoint(ibody,nfunctions,lam_x.flatten(),lam_y.flatten(),lam_z.flatten())
                if 'rigid' in body.motion_type:
                    self.fun3d_client.input_rigid_transform(ibody,body.rigid_transform)
          ...
          except FUN3DAeroException, e:	# May be Terminal Condition
            if e.code != 0:
              raise
          ...
          for ibody, body in enumerate(bodies,1):
            ...
            if 'rigid' in body.motion_type:
                body.dGdT = self.fun3d_client.extract_rigid_adjoint_product(nfunctions) * self.flow_dt
    ...
    def step_solver(self,scenario,bodies,step,fsi_subiter):
        ...
        try:
          ...
          for ibody, body in enumerate(bodies,1):
            if body.aero_nnodes > 0:
                forces.fx, forces.fy, forces.fz = self.fun3d_client.extract_forces(ibody,body.aero_nnodes)
