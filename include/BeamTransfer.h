...
class BeamTransfer : public LDTransferScheme {
 ...
 private:
  F2FScalar findParametricPoint(const F2FScalar X1[], const F2FScalar X2[],
                                const F2FScalar Xa[], double *xi);
  void computeRotation(const F2FScalar *q, const F2FScalar *d, F2FScalar r[]);
  void computeRotationTranspose(const F2FScalar *q, const F2FScalar *d,
                                F2FScalar r[]);
  void computeRotationDerivProduct(const F2FScalar *v, const F2FScalar *q,
                                   const F2FScalar *d, F2FScalar r[]);
  void addTransposeRotationDeriv(const double s, const F2FScalar *q,
                                 const F2FScalar *d, const F2FScalar *fa,
                                 F2FScalar fs[]);
  void addTransposeRotationDerivAdjoint(const double scale, const F2FScalar *q,
                                        const F2FScalar *fa,
                                        const F2FScalar *fs, F2FScalar psi[]);
