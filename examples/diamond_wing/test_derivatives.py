...
# now run the complex step
orig_DVs = np.array(struct_DVs)
n_dvs = len(struct_DVs)
rand_perturb = np.random.rand((n_dvs))
new_DVs = orig_DVs + rand_perturb * complex_step_size * 1j
model.set_variables(list(new_DVs))

# run in complex mode
driver.solve_forward()
new_functions = model.get_functions()
...
if not run_complex_only:
    ...
    complex_total_derivatives = []
    adjoint_total_derivatives = []
    for ifunc,new_func in enumerate(new_functions):
    ...
    # print report of the f2f run to terminal
    comm.Barrier()
    if comm.Get_rank() == 0:
        print(f"\nFinished funtofem analysis of {model_name}...")
        print(f"\tran in {tot_time} seconds")
        for ifunc,new_func in enumerate(new_functions):
            print(f"Derivatives for {new_func.name}...")
            print(f"\tComplex total derivative = {complex_total_derivatives[ifunc]}")
            print(f"\tAdjoint total derivative = {adjoint_total_derivatives[ifunc]}")