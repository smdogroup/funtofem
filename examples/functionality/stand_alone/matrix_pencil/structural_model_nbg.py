...
class SpringStructure(SolverInterface):
    def __init__(
        ...
        self.K = np.array([[self.k_h, 0.0], [0.0, self.k_alpha]], dtype=self.dtype)
        self.M = np.array(
            [[self.m, self.S_alpha], [self.S_alpha, self.I_alpha]], dtype=self.dtype
        )
    ...
    def iterate_eom(self, step, f):
        ...
        if time.real < self.t_f:
            ...
            u = (
                self.q[step - 1]
                + self.dt * self.qdot[step - 1]
                + b1 * self.qddot[step - 1]
                + b2 * uddot
            )
    ...
    def iterate_adjoint_eom(self, step, df):
        ...
        if time.real < self.t_f:
            ...
            # Newmark-beta method
            self.dfdx[0] += self.lmbda[1] * (
                -du1 - self.dt * du1dot - b1 * du1ddot
    ...
    def get_functions(self, scenario, bodies):
        for function in scenario.functions:
            if (
                function.analysis_type == "structural"
                ...
                if self.comm.Get_rank() == 0:
                    if self.use_test_function:
                        ...
                        if len(U) < self.N:
                            self.pitch_pencil = MatrixPencil(
                                T, U, -1, self.output_levels
                            )
                        else:
                            self.pitch_pencil = MatrixPencil(
                                T, U, self.N, self.output_levels
                            )
    ...
    def iterate(self, scenario, bodies, step):
        if self.comm.Get_rank() == 0:
            ...
            for body in bodies:
                body.struct_disps = np.zeros(body.struct_nnodes * 3, dtype=TACS.dtype)
        ...
        if self.comm.rank == 0:
            with open("adjoint_hist.dat", "w") as fp:
                ...
                for i in range(len(hist)):
                    fp.write(
                        "{0} {1} {2}\n".format(
                            (i * self.dt).real, hist[i][0].real, hist[i][1].real
                        )
                    )
        ...
        if self.comm.Get_rank() == 0:
            for ibody, body in enumerate(bodies):
                ...
                if self.aeroelastic_coupling:
                    ...
                    psi_h, psi_alpha = convert_disps_transpose(
                        h, alpha, self.p, X, psi_Y
                    )
    ...
    def eval_grad(self):
        ...
        with open("adjoint_hist.dat", "w") as fp:
            hist = self.adjoint_hist[::-1]
            for i in range(len(hist)):
                fp.write("{0} {1} {2}\n".format(i * self.dt, hist[i][0], hist[i][1]))
...
def test_transfer_transpose():
    ...
    for dh in [1e-6, 5e-7, 1e-7]:
        fd_h = (
            0.5
            * (
                np.sum(psi * convert_disps(h + dh, alpha, p, X))
                - np.sum(psi * convert_disps(h - dh, alpha, p, X))
            )
            / dh
        )
        print(
            "psi_h:     %25.15e %25.15e %25.15e" % (fd_h, psi_h, (fd_h - psi_h) / fd_h)
        )

        fd_alpha = (
            0.5
            * (
                np.sum(psi * convert_disps(h, alpha + dh, p, X))
                - np.sum(psi * convert_disps(h, alpha - dh, p, X))
            )
            / dh
        )
        print(
            "psi_alpha: %25.15e %25.15e %25.15e"
            % (fd_alpha, psi_alpha, (fd_alpha - psi_alpha) / fd_alpha)
        )
...
def test_adjoint_eom():
    ...
    for exponent in np.linspace(-10, -15, 10):
        ...
        print("%25.10e %25.10e %25.10e" % (grad[0].real, fd, (grad[0].real - fd) / fd))
    ...
    for exponent in np.linspace(-10, -15, 10):
        ...
        print("%25.10e %25.10e %25.10e" % (grad[1].real, fd, (grad[1].real - fd) / fd))
