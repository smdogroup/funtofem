...
xa = np.linspace(-2.0 * c / 2.0, 5.0 * c / 2.0, wptsa)
...
# BEND
bend = np.concatenate(
    (np.zeros((bpts * cpts, 1)), np.zeros((bpts * cpts, 1)), 0.02 * Ys**2), axis=1
)
bent = twisted + bend
...
linmeld = TransferScheme.pyLinearizedMELD(
    comm, comm, 0, comm, 0, isymm, num_nearest, beta
)
...
aero_disps_reconstructed = (
    np.dot(aero_X.reshape((-1, 3)), R.T).flatten()
