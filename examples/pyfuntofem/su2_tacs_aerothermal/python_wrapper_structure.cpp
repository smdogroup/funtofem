...
passivedouble CDriver::GetVertexAreaHeatFlux(unsigned short iMarker, unsigned long iVertex){
  ...
  if(geometry_container[ZONE_0][INST_0][MESH_0]->node[iPoint]->GetDomain() && compressible){
    ...
    // don't need these lines for area heat flux
    //for (iDim = 0; iDim < nDim; iDim++)
    //  UnitNormal[iDim] = Normal[iDim]/Area;
    ...
    for(iDim=0; iDim < nDim; iDim++){
      GradT[iDim] = solver_container[ZONE_0][INST_0][MESH_0][FLOW_SOL]->GetNodes()->GetGradient_Primitive(iPoint, 0, iDim);
      // dTdn += GradT[iDim]*UnitNormal[iDim]; For area heat flux, mult. by normal instead of unit normal
      dTdn += GradT[iDim]*Normal[iDim];
    }
