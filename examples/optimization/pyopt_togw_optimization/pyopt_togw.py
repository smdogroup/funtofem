...
class crm_togw(object):
    ...
    def __init__(self):
        ...
        solvers['structural'] = CRMtacs(self.comm,self.tacs_comm,self.model,n_tacs_procs)
    ...
    def _build_model(self):
        ...
        shape_ndv = 72
        wing = MassoudBody('wing',group=0,boundary=3,ndv=72,aero_gp_file=aero_gp_file,struct_gp_file=struct_gp_file,
                           dv_file=dv_file,usd_file=usd_file,comm=self.comm)
        ...
        for i in range(thickness.size):
            wing.add_variable('structural',Variable('thickness '+ str(i),value=thickness[i],lower = 0.001, upper = 0.1))
        ...
        for i in range(shape_ndv):
            ...
            wing.set_variable('shape',index=[i],value=0.0,lower=-0.1,upper=0.1,active=active)

        model.add_body(wing)

        # cruise scenario
        cruise = Scenario('cruise',group=0,steps=3)
        cruise.set_variable('aerodynamic',name='AOA',value=3.0,lower=-15.0,upper=15.0,coupled=False)
        ...
        maneuver.set_variable('aerodynamic',name='AOA',value=4.0,lower=-15.0,upper=15.0,coupled=False)
    ...
    def area_grad(self,dv):
        ...
        area_grad[offset+0]  += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,1.,0.,0.,0.,0.,0.)
        area_grad[offset+1]  += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,0.,1.,0.,0.,0.,0.)
        area_grad[offset+13] += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,0.,0.,1.,0.,0.,0.)
        area_grad[offset+14] += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,0.,0.,0.,1.,0.,0.)
        area_grad[offset+16] += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,0.,0.,0.,0.,1.,0.)
        ...
        area_grad[offset+1]  += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,1.,0.,0.,0.,0.,0.,0.)
        area_grad[offset+3]  += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,1.,0.,0.,0.,0.,0.)
        area_grad[offset+4]  += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,0.,1.,0.,0.,0.,0.)
        area_grad[offset+10] += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,0.,0.,1.,0.,0.,0.)
        area_grad[offset+11] += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,0.,0.,0.,1.,0.,0.)
        area_grad[offset+13] += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,0.,0.,0.,0.,1.,0.)
        area_grad[offset+14] += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
        ...
        area_grad[offset+4]  += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,1.,0.,0.,0.,0.,0.,0.)
        area_grad[offset+6]  += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,1.,0.,0.,0.,0.,0.)
        area_grad[offset+8]  += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,0.,0.,1.,0.,0.,0.)
        area_grad[offset+10] += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
                                                    0.,0.,0.,0.,0.,0.,1.,0.)
        area_grad[offset+11] += self.quad_area_grad(ax,ay,bx,by,cx,cy,dx,dy,
    ...
    def quad_area_grad(self,ax,ay,bx,by,cx,cy,dx,dy,daxdx,daydx,dbxdx,dbydx,dcxdx,dcydx,ddxdx,ddydx):
        ...
        dmdx = (dcydx-daydx) / (cx-ax) + (cy-ay)/(cx-ax)**2.0 * -(dcxdx-daxdx)

        dx_intdx =((dmdx*(by-ay) + m*(dbydx-daydx) + m**2.0 * daxdx + 2.0*m*dmdx*ax -dbxdx) /(m**2.0+1.0)
                  + (m*(by-ay) + m**2.0 * ax - bx ) / ((m**2.0+1.0)**2.0) * -2.0*m         )

        dy_intdx = dmdx * (x_int - ax) + m * (dx_intdx -daxdx) + daydx

        dh1dx = ( 0.5*( (x_int-bx)**2.0 + (y_int-by)**2.0 )**-0.5 *
        ...
        dh2dx = ( 0.5*( (x_int-dx)**2.0 + (y_int-dy)**2.0 )**-0.5 *
                ( 2.0*(x_int-dx)*(dx_intdx-ddxdx) +
                  2.0*(y_int-dy)*(dy_intdx-ddydx)   )                          )

        dedx = (  0.5*( (cy-ay)**2.0 + (cx-ax)**2.0 )**-0.5 *
...
design_problem = PyOptOptimization(dp.comm, dp.eval_objcon, dp.eval_objcon_grad,number_of_steps=3)
...
for i,var in enumerate(variables):
    print('i', i)
    opt_prob.addVar(var.name,type='c',value=var.value/dp.var_scale[i],
