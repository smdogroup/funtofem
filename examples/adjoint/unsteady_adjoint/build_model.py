...
def build_model():
    ...
    wing.add_variable(
        "structural", Variable("thickness", value=0.2, lower=0.001, upper=0.1)
    )
    ...
    cruise.set_variable(
        "aerodynamic", name="AOA", value=3.0, lower=-15.0, upper=15.0, coupled=True
    )
    cruise.add_variable(
        "aerodynamic",
        Variable("dynamic pressure", value=9510.486, lower=0.001, upper=100000.0),
    )
    # cruise.set_variable('aerodynamic',name='AOA',value=3.0+1.0e-30j,lower=-15.0,upper=15.0)

    func = Function(
        "cl", analysis_type="aerodynamic", start=steps, stop=steps, averaging=True
    )
