...
class CRMServer(Server):
    def __init__(self, comm, context, endpoint, type_, ndof, model, integrator_options):
        ...
        wing = self.builder.addMITCShellBody('wing','CRM_box_2nd.bdf',0,shellStiff,isFixed=False)

        super(CRMServer,self).__init__(comm, context, endpoint, type_, ndof, model, integrator_options)
...
if __name__ == "__main__":
    ...
    server = CRMServer(comm, context=context, endpoint=endpoint, type_=zmq.REP, ndof=6, model=model, integrator_options=options)
    server.serve()
    server.close()
    context.destroy()
